...
class UsersTest < ActionDispatch::IntegrationTest
  ...
  test "add, edit and delete users" do
    ...
    assert_difference ["User.count", "ActionMailer::Base.deliveries.count"], 1 do
    ...
    @email_token_digest = Devise.token_generator.digest(self, :unlock_token, @email_token)
    ...
    assert_difference("ActionMailer::Base.deliveries.count", 1) do
      patch user_registration_url, params: { user: { current_password: "password", email: "new_email@example.com", name: "my_new_name", password: "passwordpassword", password_confirmation: "passwordpassword" } }
    end 
    @new_user.reload
    assert_redirected_to root_url
    follow_redirect!
    assert_select "div#flash_notice", "You updated your account successfully, but we need to verify your new email address. Please check your email and follow the confirm link to confirm your new email address."
    @mail = ActionMailer::Base.deliveries.last
    ...
    get new_user_session_url
    post user_session_url, params: { user: { email: @first_admin.email, password: "password" } }
    assert_redirected_to root_url
    follow_redirect!
    assert_select "div#flash_notice", "Signed in successfully."
    assert logged_in?
